...
def fmap(dataframe, var_x, var_y, stop=-1, clusters=0):
    """
    Performs PCA on the specified data set, runs k-means algorithm,
    and returns a table of factors with cluster group numbers.
    Prints out Eigen values, loadings, cluster centers, and correspondents in the process.

    :param dataframe: Entire datafile. Session ID should be the index
    :param var_x: First variable of the range to be included in the analysis.
    :param var_y: Last variable of the range to be included in the analysis.
    :param stop: Criteria to stop. Integer number indicating the number of factors to be included.
    :param clusters: Number of groups desired
    :type dataframe: pandas.Dataframe
    :type var_x: string
    :type var_y: string
    :type stop: int
    :type clusters: int
    :return: Table of factor loadings and cluster group numbers.
    :rtype: pandas.Dataframe
    ...
    import pandas as pd
    df = pd.read_excel('C:/Users/Bob/Desktop/pca data.xlsx', index_col=0)
    pt.fmap(df, 'q6_1', 'q6_37', stop=6, clusters=4)
    """
    factors = pca(dataframe, var_x, var_y, stop=stop, return_factors=True)
    if clusters > 0:
        # get kmeans
        kmeans = KMeans(n_clusters=clusters).fit(factors)\

        # save kmeans cluster centers
        kmeans_df = pd.DataFrame(kmeans.cluster_centers_)
        kmeans_df.index.name = 'group'
        new_col_names = []
        for c in range(0, len(kmeans_df.columns)):
            new_col_names.append('factor ' + str(c+1))
        kmeans_df.columns = new_col_names
        print '\nCluster Centers: '
        print kmeans_df

        # update factors table with the new cluster column
        factors['cluster'] = pd.Series(kmeans.labels_, index=factors.index)

        # normalize kmeans cluster centers
        rec_kmeans = kmeans_df.copy()
        for j in kmeans_df.columns:
            for i in kmeans_df.index:
                rec_kmeans.loc[i, j] = \
                    (kmeans_df.loc[i, j] - kmeans_df[j].min())/(kmeans_df[j].max()-kmeans_df[j].min())

        # correspondence
        ca = CA(rec_kmeans)
        print '\nCorr. Factors: '
        print pd.DataFrame(ca.F)
        print '\nCorr. Clusters: '
        print pd.DataFrame(ca.G)
